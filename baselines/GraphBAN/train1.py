...
if __name__ == '__main__':
    ...
    dataFolder = f'/home/qfchen/ProteinDrugInter/datasets/{task}/random_split_results/'
    for i_seed in seeds:
        s = time.time()
        print('*' * 25, 'No.', i_seed, '-seed', '*' * 25)

        train_path = dataFolder + f'seed_{i_seed}/random_train.csv' 
        val_path = dataFolder + f'seed_{i_seed}/random_val.csv' 
        test_path = dataFolder + f'seed_{i_seed}/random_test.csv'

        
        # 生成教师模块图的embedding
        code = os.path.join(code_file,'teacher_gae.py')
        print(code)
        teacher_path = os.path.join(path, f'teacher_emd_random_{task}_{i_seed}.parquet')
        ...
        save_dir = f'./results/{task}/random_split/{i_seed}/'
