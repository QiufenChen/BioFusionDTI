...
class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader, save_dir,
        ...
        if self.is_da:
            ...
            if config["DA"]["RANDOM_LAYER"] and not config["DA"]["ORIGINAL_RANDOM"]:
                self.random_layer = nn.Linear(
                    ...
                    out_features=config["DA"]["RANDOM_DIM"], bias=False
                ).to(self.device)
        ...
        self.early_stopper = EarlyStopping(patience=10, delta=1e-4)
    ...
    def train(self):
        ...
        for i in range(self.epochs):
            ...
            self.early_stopper.step(auroc)
            if self.early_stopper.early_stop:
                print(f"Early stopping at epoch {self.current_epoch + 1}")
                break

        auroc, auprc, f1, sensitivity, specificity, accuracy, test_loss, thred_optim, precision, recall, cm1, y_pred, _, _ = self.test(dataloader="test")
        test_lst = ["epoch " + str(self.best_epoch)] + list(map(float2str, [
            auroc, auprc, f1, sensitivity, specificity, accuracy, thred_optim, test_loss, precision, recall
        ]))
    ...
    def train_epoch(self):
        ...
        for i, (v_d, sm, v_p, esm, labels, teacher) in enumerate(tqdm(self.train_dataloader)):
            ...
            esm = torch.tensor(esm, dtype=torch.float32).reshape(sm.shape[0], 1, 1280)
            ...
            v_d, v_p, f, score = self.model(v_d, sm, v_p, esm, self.device)
            if self.n_class == 1:
                n, loss = binary_cross_entropy(score, labels)
            else:
                n, loss = cross_entropy_logits(score, labels)
            loss += F.mse_loss(teacher, f)
            loss.backward()
            self.optim.step()
    ...
    def train_da_epoch(self):
        # 保持原来的 DA 训练逻辑，这里不改 Precision / Recall 相关部分
        pass
    ...
    def test(self, dataloader="test"):
        ...
        with torch.no_grad():
            ...
            for i, (v_d, sm, v_p, esm, labels) in enumerate(data_loader):
                ...
                if dataloader == "val":
                    ...
                    v_d, v_p, f, score = self.best_model(v_d, sm, v_p, esm, self.device)
                if self.n_class == 1:
                    n, loss = binary_cross_entropy(score, labels)
                else:
                    n, loss = cross_entropy_logits(score, labels)
        ...
        if dataloader == "test":
            ...
            prec_curve, recall_curve, _ = precision_recall_curve(y_label, y_pred)
            precision_curve = tpr / (tpr + fpr)
            ...
            return auroc, auprc, np.max(f1_curve[5:]), sensitivity, specificity, accuracy, test_loss, thred_optim, precision1, recall1, cm1, y_pred, thred_optim, y_pred
        else:
...
#
#
#         test_metric_header = ["# Best Epoch", "AUROC", "AUPRC", "F1",
#                               "Sensitivity", "Specificity", "Accuracy",
#                               "Threshold", "Test_loss", "Precision"]
#
#         self.test_table = PrettyTable(test_metric_header)
#
#
...
#
# class Trainer(object):
#     def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader, save_dir, opt_da=None, discriminator=None,
#                  experiment=None, alpha=1, **config):
#         self.model = model
#         self.optim = optim
#         self.device = device
