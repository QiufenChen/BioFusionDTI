...
if __name__ == '__main__':
    ...
    log_file = os.path.join(log_dir, f'prot_{task_lower}.log')
    sys.stdout = Logger(log_file)

    seed_results = []

    input_dir = f'/home/qfchen/ProteinDrugInter/datasets/{task}/prot_split_results/'
    seeds = [42, 1234, 3407, 5678, 8888]
    for i_seed in seeds:
        print('*' * 25, 'No.', i_seed, '-seed', '*' * 25)
        train_file = input_dir + f'seed_{i_seed}/prot_train.csv'
        valid_file = input_dir + f'seed_{i_seed}/prot_val.csv'
        test_file = input_dir + f'seed_{i_seed}/prot_test.csv'
        ...
        torch.save(best_model.state_dict(),
               os.path.join(model_dir, f'prot_{task_lower}_seed_{i_seed}.pt'))
    ...
    # 保存汇总结果
    results_df.to_csv(os.path.join(result_dir, f'prot_{task_lower}_cv_results.csv'), index=False)
